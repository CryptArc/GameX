	...
	.defines(function() {
		...
		ig.BackgroundMap = ig.Map.extend({
			...
			preRenderChunk: function(cx, cy, w, h) {
				...
				var nx =
						(cx * this.chunkSize / ig.system.scale) % this.tilesize,
					ny =
						(cy * this.chunkSize / ig.system.scale) % this.tilesize;
				var tx = Math.floor(
				...
				for (
					...
					for (
						...
						if (tileX >= this.width || tileX < 0) {
							...
							tileX =
								(tileX % this.width + this.width) % this.width;
						}
		...
		ig.ImpactSplashLoader = ig.Loader.extend({
			...
			drawPaths: function(color, paths) {
				...
				for (var i = 0; i < paths.length; i += 2) {
					ctx[ig.ImpactSplashLoader.OPS[paths[i]]].apply(
						ctx,
						paths[i + 1]
					);
				}
...
ig.baked = true;
ig
	.module("plugins.canvas-css3-scaling")
...
ig
	...
	.defines(function() {
		...
		EntityAsteroid = ig.Entity.extend({
			...
			check: function(other) {
				...
				ig.game.UIShield.currentAnim =
					ig.game.UIShield.anims[
						ig.game.player.health.floor().toString()
					];
		...
		RogersGame = ig.Game.extend({
			...
			gameOverScreenAsteroid:
				animSheetList.screenGameOverAsteroid[_LANG_CODE],
			...
			update: function() {
				if (this.score >= this.maxScore) {
					...
					if (
						newHighscore > this.oldHighscore &&
						!this.settingTheScore
					) {
						ig.game.storage.setHighest(
							"rogers-highscore",
							newHighscore
						);
					...
					if (ig.input.pressed("ok")) {
						...
						if (ig.input.pressed("cheat code")) {
							...
							this.BtnBomb.currentAnim = this.BtnBomb.anims[
								"active"
							];
							this.player.health = 100;
							this.UIShield.currentAnim = this.UIShield.anims[
								"100"
							];
						}
						this.speed += 0.0005;
						...
						for (var i = 0; i < this.entities.length; i++) {
							if (this.entities[i].type == ig.Entity.TYPE.B)
								this.entities[i].pos.x -= this.entities[
									i
								].speed;
						}
						...
						if (
							this.player.pos.y >
							ig.system.height + this.player.size.y
						) {
							this.gameOver = true;
				...
				if (this.score >= this.maxScore) {
					...
					if (!this.gameOverRunOnce) {
						...
						if (this.score > this.oldHighscore) {
							this.gameOverHighscore.draw(
								ig.system.width - 22 - 96,
								142
							);
		...
		StartScreen = ig.Game.extend({
			...
			init: function() {
				...
				this.BtnBlackmoon = this.spawnEntity(
					EntityButtonBlackmoon,
					10,
					10
				);
			...
			draw: function() {
				...
				this.ship.draw(
					-30,
					ig.system.height - this.ship.height + tick + 10
				);
				this.logo.draw((ig.system.width - this.logo.width) / 2, 10);
				this.title.draw(
					(ig.system.width - this.title.width + 160) / 2,
					100
				);
