...
contract gamex {
    ...
    // total listAllSales
    function totSales() public view returns(uint) {
    ...
    modifier _registeredUser(address sender) {
        require(users[sender].is_registered);
        _;
    }

    //Check if user is already registered
    function isRegistered(address sender) public view _registeredUser(sender) returns(bytes32, uint8) {
        return (users[sender].name, users[sender].nameSize);
    }
    
    // Registration for New User
    function newUser(bytes32 name, uint8 nameSize) notRegisteredUser public returns (bool) {
        games storage abc;
        abc.is_registered = true;
        abc.name = name;
        abc.nameSize = nameSize;
    ...
    function addGame(bytes game_name) _registeredUser(msg.sender) public payable returns (bytes32) {
    ...
    function addGameCredit(address player, bytes game_name , uint game_credit_unique_number ,uint amount) /*isGameOwner(game_name)*/ _registeredUser(player) public returns(bool) { // temporarily removed isGameOwnerCheck
    ...
    function sellCredit(bytes game_name, uint game_credit_unique_number, uint sell_amount, uint total_price) _registeredUser(msg.sender) checkCreditAmount(msg.sender, game_name, game_credit_unique_number, sell_amount) public returns(bool) {
    ...
    function cancelCreditSale(uint index) _registeredUser(msg.sender) isSeller(msg.sender, index) public returns(bool){
        uint size = tot_items;
    ...
    function getMySales() public view _registeredUser(msg.sender) returns(uint[], uint[], uint[], bytes32[]){
        uint[] memory cui;
        uint[] memory ca;
        uint[] memory cp;
        bytes32[] memory g;
    ...
    function listAllSales() public view _registeredUser(msg.sender) returns(uint[], uint[], uint[], bytes32[]) {
        uint[] memory cui;
        uint[] memory ca;
        uint[] memory cp;
        bytes32[] memory g;
    ...
    function buyCredits(uint index) public payable _registeredUser(msg.sender) returns(bool) {
